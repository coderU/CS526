...
string send_M0_to_t(string M0, int EK0_len){
  ...
  ofstream myfile;
  string filename = idlog+".key";
  myfile.open (filename.c_str());
  myfile << hex_to_string_len(hex_A0, 512);
  myfile.close();
...
void send_M1_to_u(string message,string idlog){
  if(DEBUG)
...
void createlog(string filename){
  ...
  if (!RAND_bytes(a0, 256))
    {
      cout<<"RAND wrong"<<endl;
      //exit(-1);
    }
  A0 = string(a0, a0 + 256/ sizeof(a0[0]));
...
int main(int argc, char* argv[]){
  ...
  while(true){
    cout<<"Please choose a command: newlog, append, closelog, verify, verifylog, exit."<<endl;
	...
	cout<<"You have not created a log yet"<<endl;
      }
    ...
    }else if( command.compare("verifylog") == 0 ){
      if(DEBUG)
	...
	cout<<"You can not verify a whole log while a log is current openning"<<endl;
      }
